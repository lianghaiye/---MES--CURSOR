...
export function buildMaterialFromProduct(product) {
  ...
  return {
    ...
    canSell: product.canSell !== false,
    canProduce: Boolean(product.canProduce ?? product.isPart),
    canPurchase: Boolean(product.canPurchase),
    canOutsource: Boolean(product.canOutsource),
    isAssemblyPart: Boolean(product.isAssemblyPart),
    productAttribute: product.productAttribute,
    productCategoryKey: product.categoryKey,
    materialCategoryKey: matCatKey,
    matchingRequirements: product.matchingRequirements || product.remark || '',
    remark: product.matchingRequirements || product.remark || '',
    outputTaxRate: product.outputTaxRate,
    inputTaxRate: product.inputTaxRate,
...
export function buildProductFromMaterial(material) {
  ...
  return {
    ...
    isProductMaterial: true,
    canSell: material.canSell !== false,
    canProduce: material.canProduce !== false,
    isWholeMachine: Boolean(material.isWholeMachine),
    isPart: Boolean(material.isPart),
    canPurchase: Boolean(material.canPurchase),
    canOutsource: Boolean(material.canOutsource),
    isAssemblyPart: Boolean(material.isAssemblyPart),
    matchingRequirements: material.matchingRequirements || material.remark || '',
    remark: material.matchingRequirements || material.remark || '',
    outputTaxRate: material.outputTaxRate,
    inputTaxRate: material.inputTaxRate,
