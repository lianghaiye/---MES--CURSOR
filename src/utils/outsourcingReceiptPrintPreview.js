/**
 * 外协收货单打印预览：与采购订单打印同款 sessionStorage + 新窗口预览逻辑
 */

const STORAGE_PREFIX = 'outsourcing-receipt-print-preview:'
...
/** 构建单张外协收货单打印数据（含明细） */
export function buildOutsourcingReceiptPrintPayload(receipt, options = {}) {
  ...
  const basicFields = [
    ...
    { label: '外协单号', value: receipt.outsourcingOrderNo },
    { label: '供应商', value: receipt.supplier },
    { label: '联系人', value: receipt.contactPerson },
...
/** 批量：多张收货单各打一页 */
export function buildOutsourcingReceiptBatchPrintPayload(receipts, options = {}) {
  const sheets = (receipts || [])
    .map((receipt) => buildOutsourcingReceiptPrintPayload(receipt, options))
    .filter(Boolean)
...
export function openOutsourcingReceiptPrintPreview(router, payload, { autoPrint = false } = {}) {
  const key = saveOutsourcingReceiptPrintPayload(payload)
  const query = { key }
  if (autoPrint) query.autoPrint = '1'
  const { href } = router.resolve({ name: 'procurement-outsourcing-receipt-preview', query })
