...
export const RESOURCE_TYPES = ['工人', '工人小组']

export const MOCK_POSITIONS = ['机加工岗', '装配工岗', '质检岗', '仓储岗', '热处理岗', '动平衡岗']
...
export function createProcessConfigSeed() {
  const defs = [
    ['机械', '下料', { opStart: true, opFinish: true, opFeeding: true }],
    ['机械', '粗车', { opStart: true, opFinish: true }],
    ...
    ['组装', '领料', { opStart: true, opFeeding: true, opInbound: true }],
    ...
    ['组装', '总装', { opStart: true, opFinish: true, opAutoStart: true }],
    ['组装', '灌胶', { opStart: true, opFinish: true, opFeeding: true }],
    ['组装', '整机调试', { opStart: true, opFinish: true, opAttachment: true }],
    ['拆解', '拆解', { opStart: true, opFinish: true, opDisassembly: true }],
    ['拆解', '拆解质检', { opStart: true, opFinish: true, opQc: true, opDisassemblyQc: true }],
    ['拆解', '零件清洗', { opStart: true, opFinish: true }],
    ['系统工序', '质检', { opStart: true, opFinish: true, opQc: true }],
    [
      '系统工序',
      '入库',
  ...
  return defs.map(([category, name, ops], index) => ({
    id: `proc-${String(index + 1).padStart(3, '0')}`,
    code: padCode(index + 1),
    name,
    category,
    resourceType: category === '组装' ? '工人小组' : '工人',
