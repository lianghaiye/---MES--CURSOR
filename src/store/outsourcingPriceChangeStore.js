 ...
 */
export function resolveOutsourcingOrderChangeStatus(outsourcingOrderId) {
  if (!outsourcingOrderId) return '-'
  if (getPendingOutsourcingPriceChange(outsourcingOrderId)) return '变更审核中'
  const latest = listOutsourcingPriceChangesByOrderId(outsourcingOrderId)[0]
  if (!latest) return '-'
  if (latest.status === OUTSOURCING_PRICE_CHANGE_STATUS.APPROVED) return '已通过'
  if (latest.status === OUTSOURCING_PRICE_CHANGE_STATUS.REJECTED) return '已拒绝'
  return '-'
}
