...
export const AUTO_APPROVE_DESCRIPTION =
  '勾选后对应单据提交即自动审批通过。勾选「销售订单变更」后，订单变更单提交即回写基本信息与销售明细（数量、交期、税率、单价、折扣、取消行及客户等）；勾选「采购订单价格变更」后，价格变更单提交即回写采购订单有效价。未勾选「领料申请」时，领料申请单需人工审核（待审核 → 审核通过 / 审核驳回）。未勾选「库存扣减」时，待确认扣减需人工确认（编辑 / 确认 / 作废）；勾选后提交即自动确认扣减。'
