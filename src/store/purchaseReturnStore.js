...
function migrateLegacyOutboundOrders(row) {
  if (Array.isArray(row.outboundOrders) && row.outboundOrders.length) {
    return row.outboundOrders.map((o) =>
      createPurchaseReturnOutboundOrder({
        ...o,
        lineItems: (o.lineItems || []).map((l) => createPurchaseReturnOutboundLine(l)),
      }),
    )
  }
...
function initReturns() {
  const list = shouldReseed() ? clonePurchaseReturns() : loadFromStorage() || clonePurchaseReturns()
  return list.map(normalizeReturn)
}

export const purchaseReturnState = reactive({
  returns: initReturns(),
})
...
export function getPurchaseReturnById(id) {
  return purchaseReturnState.returns.find((r) => r.id === id) || null
}

export function canEditPurchaseReturn(row) {
  return row?.status === '新建'
}

export function canVoidPurchaseReturn(row) {
  return row?.status === '新建'
}

export function addPurchaseReturn(partial = {}) {
  const returnNo =
    String(partial.returnNo || '').trim() || generatePurchaseReturnNo(purchaseReturnState.returns)
  const row = createPurchaseReturn({
    ...partial,
    returnNo,
    status: '新建',
    outboundStatus: '待出库',
  ...
  purchaseReturnState.returns.unshift(normalizeReturn(row))
  return row
}

export function updatePurchaseReturn(id, patch = {}) {
  const idx = purchaseReturnState.returns.findIndex((r) => r.id === id)
  if (idx < 0) return null
  const current = purchaseReturnState.returns[idx]
  ...
  purchaseReturnState.returns[idx] = normalizeReturn(current)
  return purchaseReturnState.returns[idx]
}

export function voidPurchaseReturn(id) {
  const row = getPurchaseReturnById(id)
  if (!row) return { ok: false, message: '退货单不存在' }
  if (!canVoidPurchaseReturn(row)) {
    return { ok: false, message: '仅「新建」状态的退货单可作废' }
  }
  row.status = '作废'
  row.updater = 'admin1'
...
/**
 * 按出货仓库拆分生成退货出库单：新建 → 进行中
 * @param {string} id
 * @param {Array|{lineId,issueQty,shipWarehouse,...}|null} lines 弹窗提交明细；空则按退货清单全量出库
 * @param {{ shipDate?: string, remark?: string }} extra
 */
export function generateOutboundFromReturn(id, lines = null, extra = {}) {
  const row = getPurchaseReturnById(id)
  if (!row) return { ok: false, message: '退货单不存在' }
  if (row.status === '作废') return { ok: false, message: '已作废的退货单不可生成出库单' }
  if (row.status !== '新建') {
    return { ok: false, message: '仅「新建」状态的退货单可生成出库单' }
  }
  ...
  if (!sourceLines.length) {
    return { ok: false, message: '退货清单为空，无法生成出库单' }
  }
  ...
  for (const item of submitItems) {
    ...
    if (!line) return { ok: false, message: '存在无效的退货明细行' }
    ...
    if (qty > maxQty + 1e-9) {
      return {
        ok: false,
        message: `物料「${line.productName || line.productCode}」出库数量不能超过退货数量`,
      }
    }
  }
  ...
  let seq = nextOutboundSeq(
    purchaseReturnState.returns.flatMap((r) =>
  ...
  groups.forEach((groupLines, warehouse) => {
    seq += 1
    const outboundOrderNo = `CKTH-${stamp}-${String(seq).padStart(3, '0')}`
    orders.push(
      createPurchaseReturnOutboundOrder({
        id: `out-return-${row.id}-${seq}`,
        outboundOrderNo,
...
export function listReturnOperators() {
  const set = new Set()
  purchaseReturnState.returns.forEach((r) => {
