...
import {
  ...
  addWorkOrderScheduleBatch,
  dispatchWorkOrderScheduleBatch,
  removeWorkOrderScheduleBatch,
  setActiveWorkOrderScheduleBatch,
...
const filteredOrders = computed(() =>
  sortWorkOrdersForList(filterWorkOrders(workOrderState.orders, appliedFilters.value)),
)
...
watch(
  ...
  (wo) => {
    ...
    } else if (shouldShowWorkOrderDispatchTab(wo) && canShowDispatchTab(wo.status)) {
...
function handleDispatchAndStart() {
  ...
  if (!ok) return
  const batchResult = addWorkOrderScheduleBatch(wo.id, {
    qty: batchQty,
...
function onDetailAction({ key, workOrder: wo, record, patch }) {
  ...
  if (key === 'select-batch' && record?.id) {
    setActiveWorkOrderScheduleBatch(wo.id, record.id)
    return
  }
  if (key === 'dispatch-batch' && record?.id) {
    const result = dispatchWorkOrderScheduleBatch(wo.id, record.id)
    if (!result.ok) {
  ...
  if (key === 'remove-batch' && record?.id) {
    Modal.confirm({
      ...
      okType: 'danger',
      onOk: () => {
        const result = removeWorkOrderScheduleBatch(wo.id, record.id)
        if (!result.ok) {
...
function onScheduleBatchSubmit(payload) {
  ...
  if (!wo) return
  const result = addWorkOrderScheduleBatch(wo.id, payload)
  if (!result.ok) {
