<template>
  <div class="app-root">
    <div class="page-wrapper">
        ...
        <div class="page-card">
          ...
          <div class="toolbar">
            ...
            <div class="toolbar-right">
              <span class="selected-count" v-if="batchSelectedIds.length > 0"
                >已选 {{ batchSelectedIds.length }} 条</span
              >
            </div>
          </div>
              ...
              <tbody>
                <tr v-for="label in filteredLabels" :key="label.id">
                  <td>
                    <input
                      type="checkbox"
                      :checked="batchSelectedIds.includes(label.id)"
                      @change="toggleRow(label.id)"
                    />
                  </td>
                  <td>
                    <a class="code-link" @click="viewDetail(label)">{{ label.labelCode }}</a>
                  </td>
                  <td>
                    <span
                      class="status-tag"
                      :class="label.qrStatus === '已绑定' ? 'status-bound' : 'status-unbound'"
                    >
                      {{ label.qrStatus }}
                    </span>
                  </td>
                  <td>
                    <a class="code-link" @click="goToRequestDetail(label.requestOrderNo)">{{
                      label.requestOrderNo
                    }}</a>
                  </td>
                  ...
                  <td class="action-cell sticky-col">
                    <a class="action-link" @click="openChangeTemplateModalForSingle(label)"
                      >更新绑定信息</a
                    >
          ...
          <div class="detail-content" v-if="detailLabel">
            ...
            <div class="detail-list">
              <div class="detail-row">
                <span class="d-label">标识码</span
                ><span class="d-value">{{ detailLabel.labelCode }}</span>
                <span class="d-label">二维码状态</span
                ><span class="d-value"
                  ><span
                    class="status-tag"
                    :class="detailLabel.qrStatus === '已绑定' ? 'status-bound' : 'status-unbound'"
                    >{{ detailLabel.qrStatus }}</span
                  ></span
                >
              </div>
              <div class="detail-row">
                <span class="d-label">关联申请单号</span
                ><span class="d-value"
                  ><a class="code-link" @click="goToRequestDetail(detailLabel.requestOrderNo)">{{
                    detailLabel.requestOrderNo
                  }}</a></span
                >
                <span class="d-label">标识状态</span
                ><span class="d-value"><span class="status-tag status-success">已注册</span></span>
              </div>
              <div class="detail-row">
                <span class="d-label">产品名称</span
                ><span class="d-value">{{ detailLabel.productName }}</span>
                <span class="d-label">批次号</span
                ><span class="d-value">{{ detailLabel.batchNo }}</span>
              </div>
              <div class="detail-row">
                <span class="d-label">规格型号</span
                ><span class="d-value">{{ detailLabel.spec }}</span>
                <span class="d-label">注册时间</span
                ><span class="d-value">{{ detailLabel.regTime }}</span>
              </div>
            </div>
            ...
            <div v-if="detailTab === 'qrcode'" class="tab-content">
              <div class="qrcode-display">
                ...
                <div class="qrcode-meta">
                  ...
                  <button class="btn btn-primary btn-sm" @click="downloadSingleQr(detailLabel)">
                    下载二维码
                  </button>
                </div>
              </div>
      ...
      <!-- ============ 变更信息更新模板弹窗 ============ -->
      <div
        v-if="changeTemplateModalVisible"
        class="modal-overlay"
        @click.self="closeChangeTemplateModal"
      >
        <div class="modal-card modal-lg">
          ...
          <div class="modal-body">
            ...
            <div class="form-row" style="margin-top: 20px">
              ...
              <select
                v-model="changeTemplateForm.newTemplate"
                class="form-input"
            ...
            <div class="form-row">
              ...
              <textarea
                v-model="changeTemplateForm.reason"
                class="form-input form-textarea"
                placeholder="选填，上限100字"
                maxlength="100"
              ></textarea>
            </div>
          </div>
          ...
          <div class="modal-body">
            <div class="export-info">
              <div class="export-info-item">
                ...
                <span class="export-value"
                  >已选择的标识记录 <strong>{{ batchSelectedIds.length }}</strong> 条</span
                >
              </div>
              <div class="export-info-item">
            ...
            <div class="export-fields-section">
              ...
              <label class="export-check"
                ><input type="checkbox" v-model="exportFields.basic" disabled checked />
                标识基础信息（必选）</label
              >
              <label class="export-check"
                ><input type="checkbox" v-model="exportFields.qrcode" /> 二维码</label
              >
              <label class="export-check"
                ><input type="checkbox" v-model="exportFields.template" /> 产品更新模板信息</label
              >
            </div>
            <div class="export-name-hint">导出文件名：标识信息导出_{{ exportDateStr }}_01.xlsx</div>
          </div>
          <div class="modal-footer">
          ...
          <div class="modal-body">
            <div class="export-info">
              ...
              <div class="export-info-item">
                ...
                <span class="export-value">{{
                  batchSelectedIds.length > 1 ? 'ZIP 压缩包' : '独立 PNG 图片'
                }}</span>
              </div>
            </div>
            <div class="export-name-hint">
              {{
                batchSelectedIds.length > 1
                  ? '下载文件名：二维码下载_' + exportDateStr + '_01.zip'
                  : '单个图片命名：【标识码】_产品名称.png'
              }}
            </div>
          </div>
          <div class="modal-footer">
...
export default {
  ...
  computed: {
    ...
    currentTemplateFields() {
      ...
      const tplCode = this.availableTemplates.find((t) => t.name === this.detailLabel.templateName)
  ...
  methods: {
    ...
    openChangeTemplateModal() {
      ...
      if (hasBound) {
        this.showError(
          '校验不通过',
          '所选标识中包含已绑定产品的标识，不支持变更信息更新模板，请重新勾选。',
        )
        return
      }
    ...
    confirmChangeTemplate() {
      ...
      const newTpl = this.availableTemplates.find(
        (t) => t.code === this.changeTemplateForm.newTemplate,
      )
      ...
      this.showSuccess(
        '变更成功',
        `已成功变更 ${this.changeTemplateTargets.length} 条标识的信息更新模板为"${newName}"。`,
      )
    },
    ...
    doExport() {
      ...
      this.showSuccess(
        '导出成功',
        `已导出 ${this.batchSelectedIds.length} 条标识数据，文件：标识信息导出_${this.exportDateStr}_01.xlsx`,
      )
    },
    exportSingle(label) {
      this.showSuccess(
        '导出成功',
        `已导出标识 ${label.labelCode} 的数据，文件：标识信息导出_${this.exportDateStr}_01.xlsx`,
      )
    },
    ...
    doDownload() {
      this.downloadModalVisible = false
      const msg =
        this.batchSelectedIds.length > 1
