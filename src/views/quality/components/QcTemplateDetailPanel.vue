<template>
  <div v-if="record" class="qc-template-detail-panel">
    <template v-if="resolvedTab === 'detail'">
      <DetailSectionCard title="基本信息">
        <a-row :gutter="[24, 8]" class="basic-info">
          ...
          <a-col v-if="!record.isSystem" :span="6">
...
const props = defineProps({
  ...
  /** detail | fill；由详情页顶栏胶囊 Tab 控制 */
  activeTab: { type: String, default: 'detail' },
  /** 表格纵向滚动高度，页面详情可更大 */
  tableScrollY: { type: [Number, String], default: 460 },
})

const resolvedTab = computed(() => (props.activeTab === 'fill' ? 'fill' : 'detail'))
