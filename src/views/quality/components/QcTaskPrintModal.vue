<template>
  ...
  >
    ...
    <div class="print-tip">
      <template v-if="isBatch">已选择 {{ targetTasks.length }} 条质检单。</template>
      预览按质检模板展开各产品行的检验项明细（标准 / 实测值 / 判定），可在预览页再次调起浏览器打印。
    </div>
...
const isBatch = computed(() => targetTasks.value.length > 1)
...
function openPreview(autoPrint) {
  ...
  if (!payload) return
  openQcTaskPrintPreview(router, payload, { autoPrint })
