<template>
  ...
  >
    ...
    <div class="print-tip">
      预览包含 ECN 基本信息、变更内容明细、影响分析与审批记录，可在预览页再次调起浏览器打印。
    </div>
...
import { buildEcnPrintPayload, openEcnPrintPreview } from '@/utils/ecnPrintPreview'
...
function openPreview(autoPrint) {
  if (!props.record) {
    message.warning('暂无单据数据')
    return
  }
  const payload = buildEcnPrintPayload(props.record, props.moduleConfig, {
    paper: form.paper,
  ...
  openEcnPrintPreview(router, payload, { autoPrint })
