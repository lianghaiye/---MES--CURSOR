<template>
  <a-modal
    ...
    :mask-closable="false"
    destroy-on-close
    class="outbound-order-print-modal"
  ...
  >
    ...
    <div class="print-tip">
      <template v-if="isBatch">已选择 {{ targetOrders.length }} 条出库单。</template>
      预览包含出库单基本信息与出库明细，可在预览页再次调起浏览器打印。
    </div>
...
import {
  buildOutboundOrderBatchPrintPayload,
  buildOutboundOrderPrintPayload,
  openOutboundOrderPrintPreview,
} from '@/utils/outboundOrderPrintPreview'
...
function openPreview(autoPrint) {
  ...
  const payload =
    list.length === 1
      ? buildOutboundOrderPrintPayload(list[0], options)
      : buildOutboundOrderBatchPrintPayload(list, options)
  if (!payload) return
  openOutboundOrderPrintPreview(router, payload, { autoPrint })
  emit('update:open', false)
}
</script>

<style lang="less" scoped>
.outbound-order-print-modal {
  .print-form {
    margin-bottom: 8px;
  }
