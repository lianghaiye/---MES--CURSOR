<template>
  <a-modal
    ...
    :mask-closable="false"
    destroy-on-close
    class="inbound-order-print-modal"
  ...
  >
    ...
    <div class="print-tip">
      <template v-if="isBatch">已选择 {{ targetOrders.length }} 条入库单。</template>
      预览包含入库单基本信息与入库明细，可在预览页再次调起浏览器打印。
    </div>
...
import {
  buildInboundOrderBatchPrintPayload,
  buildInboundOrderPrintPayload,
  openInboundOrderPrintPreview,
} from '@/utils/inboundOrderPrintPreview'
...
function openPreview(autoPrint) {
  ...
  const payload =
    list.length === 1
      ? buildInboundOrderPrintPayload(list[0], options)
      : buildInboundOrderBatchPrintPayload(list, options)
  if (!payload) return
  openInboundOrderPrintPreview(router, payload, { autoPrint })
  emit('update:open', false)
}
</script>

<style lang="less" scoped>
.inbound-order-print-modal {
  .print-form {
    margin-bottom: 8px;
  }
