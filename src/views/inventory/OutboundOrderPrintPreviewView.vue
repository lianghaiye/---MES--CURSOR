<template>
  <div
    class="purchase-order-print-preview-page outbound-order-print-preview-page"
    :class="pageClass"
  >
        ...
        >
          <header class="sheet-header">
            ...
            <div class="sheet-doc-no">
              ...
              <span v-if="sheet.outboundType" class="sheet-type">（{{ sheet.outboundType }}）</span>
            </div>
          </header>
          ...
          <section class="sheet-section">
            ...
            <div v-else class="bom-empty">暂无出库明细</div>
...
import { loadOutboundOrderPrintPayload } from '@/utils/outboundOrderPrintPreview'
import { printElement } from '@/utils/browserPrint'

defineOptions({ name: 'OutboundOrderPrintPreviewView' })

const route = useRoute()
const printAreaRef = ref(null)

const payload = computed(() => loadOutboundOrderPrintPayload(route.query.key))
...
function handlePrint() {
  ...
  printElement(printAreaRef.value, {
    title: printSheets.value[0]?.docNo || '出库单',
...
<style scoped>
.outbound-order-print-preview-page {
  min-height: 100vh;
  background: #e8e8e8;
  padding-bottom: 24px;
}
