<template>
  <div
    class="purchase-order-print-preview-page inbound-order-print-preview-page"
    :class="pageClass"
  >
      ...
      <div ref="printAreaRef" class="preview-canvas">
        ...
        >
          <header class="sheet-header">
            ...
            <div class="sheet-doc-no">
              单号：{{ sheet.docNo || sheet.orderNo }}
              <span v-if="sheet.inboundType" class="sheet-type">（{{ sheet.inboundType }}）</span>
            </div>
          </header>
              ...
              <table class="sheet-table bom-table">
                ...
                <tbody>
                  <tr v-for="row in sheet.lineItems" :key="`${index}-line-${row.seq}`">
                    ...
                    <td class="cell-num">{{ row.totalPrice }}</td>
                    <td>{{ row.remark }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <div v-else class="bom-empty">暂无入库明细</div>
            <div v-if="sheet.summary" class="print-summary">
              <span>明细行数：{{ sheet.summary.lineCount || '—' }}</span>
              <span>入库数量合计：{{ sheet.summary.totalQty || '—' }}</span>
          ...
          <footer class="sheet-footer">
            ...
            <span class="footer-sign">仓管员：__________</span>
          </footer>
        </article>
      </div>
    </template>
...
import { loadInboundOrderPrintPayload } from '@/utils/inboundOrderPrintPreview'
import { printElement } from '@/utils/browserPrint'

defineOptions({ name: 'InboundOrderPrintPreviewView' })

const route = useRoute()
const printAreaRef = ref(null)

const payload = computed(() => loadInboundOrderPrintPayload(route.query.key))
...
function handlePrint() {
  ...
  printElement(printAreaRef.value, {
    title: printSheets.value[0]?.docNo || '入库单',
...
<style scoped>
.inbound-order-print-preview-page {
  min-height: 100vh;
  background: #e8e8e8;
  padding-bottom: 24px;
}
