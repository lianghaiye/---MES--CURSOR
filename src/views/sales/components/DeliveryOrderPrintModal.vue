<template>
  <a-modal
    ...
    :mask-closable="false"
    destroy-on-close
    class="delivery-order-print-modal"
  ...
  >
    ...
    <div class="print-tip">
      <template v-if="isBatch">已选择 {{ targetOrders.length }} 条发货单。</template>
      预览包含发货单基本信息与发货明细，可在预览页再次调起浏览器打印。
    </div>
...
import {
  buildDeliveryOrderBatchPrintPayload,
  buildDeliveryOrderPrintPayload,
  openDeliveryOrderPrintPreview,
} from '@/utils/deliveryOrderPrintPreview'
...
const targetOrders = computed(() => {
  if (props.deliveryOrders?.length) return props.deliveryOrders
  return props.deliveryOrder ? [props.deliveryOrder] : []
})
...
function openPreview(autoPrint) {
  ...
  const payload =
    list.length === 1
      ? buildDeliveryOrderPrintPayload(list[0], options)
      : buildDeliveryOrderBatchPrintPayload(list, options)
  if (!payload) return
  openDeliveryOrderPrintPreview(router, payload, { autoPrint })
