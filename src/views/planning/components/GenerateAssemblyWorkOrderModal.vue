<template>
  ...
  >
        ...
        <template #bodyCell="{ column, record, text }">
          ...
          >
            <div v-if="isEditing(record.key, column.key)" class="edit-wrap" @click.stop>
              ...
              />
              <a-input-number
                v-else-if="column.key === 'planQty' || column.key === 'scheduleQty'"
                v-model:value="record[column.key]"
                size="small"
                :min="0"
                style="width: 100%"
    ...
    <a-empty
      v-if="!rows.length"
      description="当前订单无供应型态为「组装」的物料，或顶级物料未设为「组装」"
    />
...
import { processRouteOptions, workCenterOptions, urgencyOptions } from '@/mock/workOrderOptions'
import { getWarehouseSelectOptions, warehouseState } from '@/store/warehouseStore'
import { buildAssemblyWorkOrderRows } from '@/utils/material'
...
const columnDefs = [
  ...
  { key: 'orderCategory', title: '工单类型', width: 100, total: false },
  { key: 'productName', title: '产品名称', width: 130, total: false },
  ...
  { key: 'scheduleQty', title: '排产数量', width: 90, editable: true, total: true, numeric: true },
  ...
  columnDefs
    ...
    .map((c) => ({
      ...
      fixed:
        c.key === 'action'
          ? 'right'
          : ['index', 'orderCategory', 'productName', 'code'].includes(c.key)
            ? 'left'
            : undefined,
    })),
)
...
watch(
  ...
  (val) => {
    if (val && props.order) {
      rows.value = buildAssemblyWorkOrderRows(props.materials, props.order)
...
function formatCell(record, key, text) {
  ...
  if (isEditable(key) && (text === '' || text == null)) {
    if (['processRoute', 'workCenter'].includes(key)) return '请选择'
    return '-'
  }
  return text ?? '-'
}
