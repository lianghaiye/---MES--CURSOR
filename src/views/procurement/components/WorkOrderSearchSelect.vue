<template>
  ...
  <WorkOrderSelectModal v-model:open="pickerOpen" @confirm="onPickedFromModal" />
</template>
...
import {
  WORK_ORDER_DROPDOWN_QUICK_LIMIT,
  WORK_ORDER_SELECT_PLACEHOLDER,
  buildWorkOrderDisplayOptions,
  getAllWorkOrderOptions,
} from '@/utils/workOrderPicker'
import WorkOrderSelectModal from '@/views/engineering-change/components/WorkOrderSelectModal.vue'

const props = defineProps({
  value: { type: String, default: '' },
  placeholder: { type: String, default: WORK_ORDER_SELECT_PLACEHOLDER },
...
const allOptions = computed(() => getAllWorkOrderOptions(workOrderState.orders))
...
const displayOptions = computed(() =>
  buildWorkOrderDisplayOptions({
    ...
    quickLimit: WORK_ORDER_DROPDOWN_QUICK_LIMIT,
  }),
)
...
function onPickedFromModal(workOrder) {
  if (!workOrder?.code) return
  emit('update:value', workOrder.code)
  emit('change', workOrder.code)
}
</script>

<script>
export default { name: 'WorkOrderSearchSelect' }
</script>
