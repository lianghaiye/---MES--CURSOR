<template>
  <a-modal
    ...
    :mask-closable="false"
    destroy-on-close
    class="outsourcing-receipt-print-modal"
  ...
  >
    ...
    <div class="print-tip">
      <template v-if="isBatch">已选择 {{ targetReceipts.length }} 条收货单。</template>
      预览包含收货单基本信息与收货明细，可在预览页再次调起浏览器打印。
    </div>
...
import {
  buildOutsourcingReceiptBatchPrintPayload,
  buildOutsourcingReceiptPrintPayload,
  openOutsourcingReceiptPrintPreview,
} from '@/utils/outsourcingReceiptPrintPreview'
...
const targetReceipts = computed(() => {
  if (props.receipts?.length) return props.receipts
  return props.receipt ? [props.receipt] : []
})

const isBatch = computed(() => targetReceipts.value.length > 1)
...
function openPreview(autoPrint) {
  const list = targetReceipts.value
  ...
  const payload =
    list.length === 1
      ? buildOutsourcingReceiptPrintPayload(list[0], options)
      : buildOutsourcingReceiptBatchPrintPayload(list, options)
  if (!payload) return
  openOutsourcingReceiptPrintPreview(router, payload, { autoPrint })
  emit('update:open', false)
}
</script>

<style lang="less" scoped>
.outsourcing-receipt-print-modal {
  .print-form {
    margin-bottom: 8px;
  }
