<template>
  <a-modal
    ...
    :mask-closable="false"
    destroy-on-close
    class="purchase-return-print-modal"
  ...
  >
    ...
    <div class="print-tip">
      <template v-if="isBatch">已选择 {{ targetReturns.length }} 条退货单。</template>
      预览包含退货单基本信息与退货明细，可在预览页再次调起浏览器打印。
    </div>
...
import {
  buildPurchaseReturnBatchPrintPayload,
  buildPurchaseReturnPrintPayload,
  openPurchaseReturnPrintPreview,
} from '@/utils/purchaseReturnPrintPreview'

const props = defineProps({
  open: Boolean,
  purchaseReturn: { type: Object, default: null },
...
const targetReturns = computed(() => {
  if (props.purchaseReturns?.length) return props.purchaseReturns
  return props.purchaseReturn ? [props.purchaseReturn] : []
})
...
function openPreview(autoPrint) {
  ...
  const payload =
    list.length === 1
      ? buildPurchaseReturnPrintPayload(list[0], options)
      : buildPurchaseReturnBatchPrintPayload(list, options)
  if (!payload) return
  openPurchaseReturnPrintPreview(router, payload, { autoPrint })
  emit('update:open', false)
}
</script>

<style lang="less" scoped>
.purchase-return-print-modal {
  .print-form {
    margin-bottom: 8px;
  }
