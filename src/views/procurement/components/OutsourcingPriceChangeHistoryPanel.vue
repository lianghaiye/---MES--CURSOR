    ...
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'status'">
          <a-tag :color="outsourcingPriceChangeStatusColor(record.status)">{{
            record.status
          }}</a-tag>
        </template>
        ...
        <template v-else-if="column.key === 'newAmountExTax'">
          {{ formatOutsourcingPriceChangeAbsMoney(record.newAmountExTax) }}
        </template>
        <template v-else-if="column.key === 'newAmountInTax'">
          {{ formatOutsourcingPriceChangeAbsMoney(record.newAmountInTax) }}
        </template>
        <template v-else>
          ...
          <template #bodyCell="{ column, record: line }">
            ...
            <template v-else-if="column.key === 'deltaAmountExTax'">
              <span :class="deltaClass(line.deltaAmountExTax)">
                {{ formatOutsourcingPriceChangeMoney(line.deltaAmountExTax) }}
              </span>
            </template>
            <template v-else-if="column.key === 'deltaAmountInTax'">
              <span :class="deltaClass(line.deltaAmountInTax)">
                {{ formatOutsourcingPriceChangeMoney(line.deltaAmountInTax) }}
              </span>
            </template>
            <template v-else>
...
<script>
export default { name: 'OutsourcingPriceChangeHistoryPanel' }
</script>
...
const records = computed(() => {
  ...
  return listOutsourcingPriceChangesByOrderId(props.order?.id).map((row) =>
    normalizeOutsourcingPriceChangeRecord(row),
  )
})
...
const lineColumns = [
  ...
  { title: '计费方式', dataIndex: 'billingMethod', width: 100 },
