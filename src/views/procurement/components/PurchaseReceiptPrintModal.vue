<template>
  <a-modal
    ...
    :mask-closable="false"
    destroy-on-close
    class="purchase-receipt-print-modal"
...
import {
  buildPurchaseReceiptBatchPrintPayload,
  buildPurchaseReceiptPrintPayload,
  openPurchaseReceiptPrintPreview,
} from '@/utils/purchaseReceiptPrintPreview'
...
function openPreview(autoPrint) {
  ...
  const payload =
    list.length === 1
      ? buildPurchaseReceiptPrintPayload(list[0], options)
      : buildPurchaseReceiptBatchPrintPayload(list, options)
  if (!payload) return
  openPurchaseReceiptPrintPreview(router, payload, { autoPrint })
