<template>
  <a-modal
    ...
    :mask-closable="false"
    destroy-on-close
    class="purchase-order-print-modal"
  ...
  >
    ...
    <div class="print-tip">
      <template v-if="isBatch">已选择 {{ targetOrders.length }} 条采购订单。</template>
      预览包含采购订单基本信息与采购明细，可在预览页再次调起浏览器打印。
    </div>
...
import {
  buildPurchaseOrderBatchPrintPayload,
  buildPurchaseOrderPrintPayload,
  openPurchaseOrderPrintPreview,
} from '@/utils/purchaseOrderPrintPreview'
...
const targetOrders = computed(() => {
  if (props.purchaseOrders?.length) return props.purchaseOrders
...
function openPreview(autoPrint) {
  ...
  const payload =
    orders.length === 1
      ? buildPurchaseOrderPrintPayload(orders[0], options)
      : buildPurchaseOrderBatchPrintPayload(orders, options)
  if (!payload) return
  openPurchaseOrderPrintPreview(router, payload, { autoPrint })
