<template>
  <div class="outsourcing-receipt-basic-section">
    <div class="meta-bar">
...
const fields = computed(() => [
  { key: 'receiptNo', label: '收货单号' },
  { key: 'outsourcingOrderNo', label: '外协单号', slot: 'outsourcingOrderNo' },
  { key: 'supplier', label: '供应商' },
  { key: 'contactPerson', label: '联系人', format: (r) => display(r.contactPerson || r.purchaser) },
  { key: 'qcNo', label: '质检单号' },
  { key: 'inboundOrderNo', label: '入库单号' },
  { key: 'remark', label: '备注', fullRow: true },
])
</script>

<script>
export default { name: 'OutsourcingReceiptBasicInfoSection' }
</script>

<style lang="less" scoped>
@label-width: 108px;

.outsourcing-receipt-basic-section {
  padding: 10px 12px;
  background: #fff;
