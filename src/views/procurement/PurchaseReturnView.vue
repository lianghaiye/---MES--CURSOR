<template>
  <div class="purchase-return-page">
        ...
        <a-row :gutter="[12, 8]" style="width: 100%">
          ...
          <a-col :xs="24" :sm="12" :md="6">
            <a-form-item label="出货仓库">
              <a-select
                v-model:value="filters.shipWarehouse"
                allow-clear
                size="small"
                placeholder="请选择"
        ...
        <a-dropdown>
          ...
          <template #overlay>
            <a-menu @click="onPrintMenuClick">
              <a-menu-item key="打印退货明细">打印退货明细</a-menu-item>
            </a-menu>
          </template>
        </a-dropdown>
      </a-space>
        ...
        <template #bodyCell="{ column, record, index }">
          ...
          <template v-else-if="column.key === 'action'">
            <a-space :size="0">
              ...
              <a-button
                v-if="canVoidPurchaseReturn(record)"
                type="link"
                size="small"
                danger
              ...
              <span
                v-if="!canEditPurchaseReturn(record) && !canVoidPurchaseReturn(record)"
    ...
    <PurchaseReturnPrintModal v-model:open="printModalOpen" :purchase-returns="printReturns" />
...
<script>
export default { name: 'PurchaseReturnView' }
</script>
...
import {
  ...
  purchaseReturnOutboundStatusOptions,
...
import { supplierOptions, warehouseOptions } from '@/mock/purchaseOrderOptions'
...
const statusOpts = purchaseReturnStatusOptions.map((v) => ({ label: v, value: v }))
const outboundStatusOpts = purchaseReturnOutboundStatusOptions.map((v) => ({
  label: v,
  value: v,
}))
const supplierOpts = supplierOptions
const warehouseOpts = warehouseOptions
const operatorOpts = computed(() => listReturnOperators())
...
const baseColumns = [
  ...
  { title: '退货数量', key: 'qtySummary', width: 120 },
  { title: '出货仓库', dataIndex: 'shipWarehouse', width: 110 },
...
const { columnSettings, columnDrawerOpen, displayColumns, tableScrollX, defaultColumnSettings } =
  useTableColumnSettings('purchase-return-list-v3', baseColumns)

const filteredList = computed(() =>
  filterPurchaseReturns(purchaseReturnState.returns, appliedFilters.value),
)
...
function handleReset() {
  ...
  filters.shipWarehouse = undefined
...
function openCreate() {
  const page = findCreatePageByListPath('/procurement/purchase-returns')
  if (!page) return
...
function openDetail(record) {
  if (!record?.id) return
  const path = `/procurement/purchase-returns/${record.id}`
  openTab(path, `采购退货 ${record.returnNo || ''}`)
  router.push({ name: 'procurement-purchase-returns-detail', params: { id: record.id } })
}
...
function handleEdit(record) {
  if (!canEditPurchaseReturn(record)) {
    message.warning('仅「新建」状态的退货单可编辑')
    return
  }
  const path = `/procurement/purchase-returns/${record.id}/edit`
  openTab(path, `编辑退货单 ${record.returnNo || ''}`)
  router.push({ name: 'procurement-purchase-returns-edit', params: { id: record.id } })
}

function handleVoid(record) {
  if (!canVoidPurchaseReturn(record)) {
    message.warning('仅「新建」状态的退货单可作废')
    return
  }
  Modal.confirm({
    title: '确认作废',
    content: `确定作废退货单「${record.returnNo}」吗？`,
    okType: 'danger',
    onOk: () => {
      const result = voidPurchaseReturn(record.id)
...
function handleGenerateOutbound() {
  if (selectedRowKeys.value.length !== 1) {
    message.warning('请勾选一条「新建」状态的退货单后再生成出库单')
    return
  }
  const record = purchaseReturnState.returns.find((r) => r.id === selectedRowKeys.value[0])
  if (!record || record.status !== '新建') {
    message.warning('仅「新建」状态的退货单可生成出库单')
    return
  }
...
function openBatchPrint() {
  ...
  printReturns.value = selectedRowKeys.value
    .map((id) => purchaseReturnState.returns.find((r) => r.id === id))
    .filter(Boolean)
  if (!printReturns.value.length) {
    message.warning('未找到可打印的退货单')
    return
  }
