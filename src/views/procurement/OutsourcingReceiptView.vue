<template>
  <div class="outsourcing-receipt-page">
        ...
        <template #bodyCell="{ column, record, index }">
          ...
          <template v-else-if="column.key === 'action'">
            <a-space :size="0">
              ...
              <a-button
                v-if="canEditOutsourcingReceipt(record)"
                type="link"
                size="small"
              ...
              <a-button
                v-if="canVoidOutsourcingReceipt(record)"
                type="link"
                size="small"
                danger
    ...
    <OutsourcingReceiptPrintModal v-model:open="printModalOpen" :receipts="printReceipts" />

    <OutsourcingGenerateInboundModal
      v-model:open="inboundModalOpen"
      :outsourcing-order="inboundOrder"
      :outsourcing-receipt="inboundReceipt"
...
<script>
export default { name: 'OutsourcingReceiptView' }
</script>
...
const { columnSettings, columnDrawerOpen, displayColumns, tableScrollX, defaultColumnSettings } =
  useTableColumnSettings('outsourcing-receipt-list-v5', baseColumns)

const filteredList = computed(() =>
  filterOutsourcingReceipts(outsourcingReceiptState.receipts, appliedFilters.value),
)
...
function openDetail(record) {
  if (!record?.id) return
  const path = `/procurement/outsourcing-receipts/${record.id}`
  openTab(path, `外协收货 ${record.receiptNo || ''}`)
  router.push({ name: 'procurement-outsourcing-receipts-detail', params: { id: record.id } })
}
...
/** 新建/进行中且入库未完成、关联外协单仍可入库；有关联质检时须通过门控 */
...
function hasRowActions(record) {
  return (
    canEditOutsourcingReceipt(record) ||
    canVoidOutsourcingReceipt(record) ||
...
function handleEdit(record) {
  if (!canEditOutsourcingReceipt(record)) {
...
function handleVoid(record) {
  if (!canVoidOutsourcingReceipt(record)) {
  ...
  Modal.confirm({
    ...
    okType: 'danger',
    onOk: () => {
      const result = voidOutsourcingReceipt(record.id)
...
function handleBatchVoid() {
  ...
  const targets = selectedRowKeys.value
    ...
    .filter(Boolean)
  const voidable = targets.filter(canVoidOutsourcingReceipt)
  if (!voidable.length) {
  ...
  Modal.confirm({
    ...
    onOk: () => {
      ...
      voidable.forEach((row) => {
        const result = voidOutsourcingReceipt(row.id)
  ...
  Modal.confirm({
    ...
    onOk: () => {
      ...
      completable.forEach((row) => {
        const result = completeOutsourcingReceipt(row.id)
        if (result.ok) okCount += 1
      })
