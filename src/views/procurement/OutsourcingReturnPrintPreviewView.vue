<template>
  <div
    class="purchase-order-print-preview-page outsourcing-return-print-preview-page"
    :class="pageClass"
  >
      ...
      <div ref="printAreaRef" class="preview-canvas">
        ...
        >
          <header class="sheet-header">
            ...
            <div class="sheet-doc-no">单号：{{ sheet.returnNo || sheet.orderNo }}</div>
          </header>
              ...
              <table class="sheet-table bom-table">
                ...
                <tbody>
                  <tr v-for="row in sheet.lineItems" :key="`${index}-line-${row.seq}`">
                    ...
                    <td class="cell-num">{{ row.receivedQty }}</td>
                    <td class="cell-num">{{ row.returnQty }}</td>
                    <td>{{ row.unit }}</td>
                    <td>{{ row.returnType }}</td>
                    <td>{{ row.compensationMethod }}</td>
                    <td class="cell-num">{{ row.compensationAmount }}</td>
                    <td>{{ row.remark }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <div v-else class="bom-empty">暂无异常处理明细</div>
            <div v-if="sheet.summary" class="print-summary">
              <span>明细行数：{{ sheet.summary.lineCount || '—' }}</span>
              <span>处理数量合计：{{ sheet.summary.totalQty || '—' }}</span>
            </div>
          </section>
...
import { loadOutsourcingReturnPrintPayload } from '@/utils/outsourcingReturnPrintPreview'
import { printElement } from '@/utils/browserPrint'

const route = useRoute()
const printAreaRef = ref(null)

const payload = computed(() => loadOutsourcingReturnPrintPayload(route.query.key))
...
function handlePrint() {
  ...
  printElement(printAreaRef.value, {
    title: printSheets.value[0]?.returnNo || '外协异常处理明细',
...
<style scoped>
.outsourcing-return-print-preview-page {
  min-height: 100vh;
  background: #e8e8e8;
  padding-bottom: 24px;
}
