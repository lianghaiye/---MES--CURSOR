<template>
  <div
    class="purchase-order-print-preview-page outsourcing-receipt-print-preview-page"
    :class="pageClass"
  >
      ...
      <div ref="printAreaRef" class="preview-canvas">
        ...
        >
          <header class="sheet-header">
            ...
            <div class="sheet-doc-no">单号：{{ sheet.receiptNo || sheet.orderNo }}</div>
          </header>
              ...
              <table class="sheet-table bom-table">
                ...
                <tbody>
                  <tr v-for="row in sheet.lineItems" :key="`${index}-line-${row.seq}`">
                    ...
                    <td class="cell-num">{{ row.receiptQty }}</td>
                    <td>{{ row.unit }}</td>
                    <td>{{ row.receivingWarehouse }}</td>
                    <td>{{ row.receivingMode }}</td>
                    <td>{{ row.remark }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <div v-else class="bom-empty">暂无收货明细</div>
            <div v-if="sheet.summary" class="print-summary">
              <span>明细行数：{{ sheet.summary.lineCount || '—' }}</span>
              <span>收货数量合计：{{ sheet.summary.totalQty || '—' }}</span>
            </div>
          </section>
...
import { loadOutsourcingReceiptPrintPayload } from '@/utils/outsourcingReceiptPrintPreview'
import { printElement } from '@/utils/browserPrint'

const route = useRoute()
const printAreaRef = ref(null)

const payload = computed(() => loadOutsourcingReceiptPrintPayload(route.query.key))
...
function handlePrint() {
  ...
  printElement(printAreaRef.value, {
    title: printSheets.value[0]?.receiptNo || '外协收货明细',
...
<style scoped>
.outsourcing-receipt-print-preview-page {
  min-height: 100vh;
  background: #e8e8e8;
  padding-bottom: 24px;
}
