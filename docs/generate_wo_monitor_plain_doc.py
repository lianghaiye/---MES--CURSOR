...
def build():
    ...
    add_bullet(doc, "上面一排数字（KPI）：工单数量、任务执行、物料消耗")
    ...
    add_table(
        ...
        [
            ...
            [
                "工序移动任务 / 工时管理",
                "任务执行（任务数、良不良、工时等）、工人忙闲",
            ],
            ["领料申请 / 库存扣减", "物料消耗（已领取、待出库、已消耗）"],
            ["员工小组", "右边小组列表"],
        ],
    )
    add_para(
    ...
    add_table(
        ...
        [
            [
                "周期（本日/本周/本月）",
                "影响工单总数（按创建）、已完成工单、任务数/良不良/工时、物料消耗。待下发/进行中以及任务执行里的已报工/在制/待领取是快照，不跟周期走。",
            ],
            ["工单类型", "只看生产、只看组装，还是都看。"],
            ["工作中心", "只看某个车间/中心的工单。"],
            [
    ...
    add_bullet(doc, "工单总数：创建时间落在所选周期内的工单有多少张（什么状态都算，含终止）")
    ...
    add_heading(doc, "5.2 任务执行", 2)
    add_para(doc, "把原来的「报工数量」和「工序状态」合在一块：")
    add_bullet(doc, "计划数 / 排产数：周期内新创建工单的计划数量、排产数量合计（和「工单总数」同一批单）")
    add_bullet(doc, "任务数：周期内新生成的移动任务条数")
    add_bullet(doc, "已报工 / 在制 / 待领取：当前工单工艺节点状态快照（不跟周期）")
    add_bullet(doc, "良品数 / 不良品数：周期内任务按工单取最大值再跨单相加")
    add_bullet(doc, "工时：优先用工时管理核算小时数；没有则用任务报工时长合计")

    add_heading(doc, "5.3 物料消耗", 2)
    add_para(doc, "按物料「项数」统计，不把不同单位的数量加在一起：")
    add_bullet(doc, "已领取：领料已出库的物料行有多少项")
    add_bullet(doc, "待出库：还在待出库的物料行有多少项")
    add_bullet(doc, "已消耗：库存扣减确认成功的物料行有多少项")

    add_heading(doc, "5.4 补充说明", 2)
    add_para(
        doc,
        "现网约定：工人提交一次报工，这道工序任务就算完成——"
        "不要求报工数量必须凑满排产数。",
    )
